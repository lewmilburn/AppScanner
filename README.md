...
| `--save-list` | `-v`  | `py AppScanner.py -cl --save-list`      | Instead of downloading the apps right away, AppScanner will create a wordlist. This allows you to select apps from multiple categories and then pass the list in using the --list flag instead. If file exists, it will ask you if you want to overwrite or append. |
